...
const DEBUG = false;
const log = (tag, ...args) => DEBUG && console.log(`[CURSOR_PROTO:${tag}]`, ...args);
...
export function encodeMessage(content, role, messageId, chatModeEnum = null, isLast = false, hasTools = false, toolResults = []) {
  return concatArrays(
    ...
    encodeField(FIELD.MSG_UNIFIED_MODE, WIRE_TYPE.VARINT, chatModeEnum ?? UNIFIED_MODE.CHAT),
    encodeField(FIELD.MSG_SUPPORTED_TOOLS, WIRE_TYPE.VARINT, hasTools ? 1 : 0)
  );
}
...
export function encodeInstruction(text) {
  return encodeField(FIELD.INSTRUCTION_TEXT, WIRE_TYPE.LEN, text);
}
...
function extractToolCall(toolCallPayload) {
  const fields = decodeMessage(toolCallPayload);

  if (!fields.has(FIELD.TOOL_NAME)) return null;

  const toolName = new TextDecoder().decode(fields.get(FIELD.TOOL_NAME)[0].value);
  const toolId = fields.has(FIELD.TOOL_ID)
    ? new TextDecoder().decode(fields.get(FIELD.TOOL_ID)[0].value)
    : `tool_${Math.random().toString(36).slice(2)}`;

  let rawArgs = '';
  if (fields.has(FIELD.TOOL_RAW_ARGS)) {
    rawArgs = new TextDecoder().decode(fields.get(FIELD.TOOL_RAW_ARGS)[0].value);
  }

  let isLast = false;
  if (fields.has(FIELD.TOOL_IS_LAST)) {
    isLast = fields.get(FIELD.TOOL_IS_LAST)[0].value === 1;
  }
