...
/**
 * Ensure Codex account manager is initialized
 */
async function ensureCodexInitialized() {
    ...
    codexInitPromise = (async () => {
        await codexAccountManager.initialize();
    ...
    try {
        ...
        try {
            ...
            if (account) {
                ...
                combined.data.push(...(models.data || []));
            }
        } catch (error) {
            logger.warn('[API] Unable to load Antigravity models:', error.message);
        }
...
app.post('/v1/messages', async (req, res) => {
    try {
        ...
        if (modelFamily === 'github') {
            ...
            if (!isValidCodexModel(modelId, CODEX_MODELS)) {
                throw new Error(`invalid_request_error: Invalid model: ${modelId}. Use /v1/models to see available models.`);
            }
        }
