...
| `content[].type = "image"` (`base64`/`url`) | `message.content[].type = "input_image"` |
| `content[].type = "document"` (`base64`/`url`) | `message.content[].type = "input_file"` |
...
**Lưu ý ảnh/file local:**  
Không thể gửi đường dẫn file local dưới dạng text rồi mong model tự đọc (ví dụ `"/home/user/a.jpg"`).  
Bạn cần gửi ảnh/file dưới dạng block `image`/`document` trong `messages[].content` (base64 hoặc URL).
